...
// MaskReveal — animates text by sliding it up from behind a hidden clip
// Each line of the headline uses this for a staggered reveal effect
...
// Hero — full-screen section with parallax image and animated headline
export default function Hero() {
  const containerRef = useRef(null)
  // useScroll tracks how far we've scrolled through the hero section
  const { scrollYProgress } = useScroll({ target: containerRef, offset: ['start start', 'end start'] })
  const shouldReduce = useReducedMotion()

  // Parallax: image moves slightly slower than the scroll speed
    ...
    >
      ...
      >
        <Image
          ...
          sizes="100vw"
        />
        {/* Gradient overlays to make white text readable on the image */}
      ...
      <div className="relative z-10 h-full flex flex-col justify-end pb-16 md:pb-20 container-drave">
        <div className="max-w-xl">
          ...
          {/* Headline — each line reveals with a mask animation */}
          <div className="mb-6">
      ...
      {/* Scroll indicator — animated line on the right side */}
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
